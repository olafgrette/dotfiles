...
When you encounter an image or scanned document that needs to be identified or read, use `agy` — it has strong multimodal capabilities and can be called with `--print` for headless operation.
...
- Use `--print` for non-interactive operation
...
- **Batch identification**: Run one `agy` call per file — don't pass multiple `@` references in one call
