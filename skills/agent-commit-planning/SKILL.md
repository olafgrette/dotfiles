...
6. **Complete**: when every requirement is satisfied, required verification passes, and no slices remain, mark the plan complete.

Do not invent a target architecture from requirements alone. If the repository or a material human decision is unavailable, record the missing input and stop at a coarse slice sketch; do not fabricate file paths, contracts, or executable briefs.
...
After inspection, propose the smallest plausible target file tree. Treat a user-supplied size range as context or a ceiling, never as a minimum. For every file and abstraction, name the current requirement, repository convention, consumer, or observed pain that justifies it. Delete anything without one.
...
1. compares actual behavior and files with estimates;
...
After an executor stop, inspect the evidence and propose the smallest plan or contract amendment. Update affected downstream slices. Land an approved contract change explicitly before dependent work; never let it drift into an unrelated implementation slice.

Finish by setting the plan state to `complete`, recording final verification, and leaving no pending or executable briefs.
